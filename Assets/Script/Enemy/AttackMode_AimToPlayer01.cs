...
public class AttackMode_AimToPlayer01 : MonoBehaviour, IAttackMode
{
    ...
    void Start()
    {
        ...
        chargeBackCount = attackStartTime;
    ...
    public void Attack()
    {
        if (Time.frameCount > chargeBackCount)
        {
            for (int i = 0; i < bullentWave; i++)   //进行数波发射
            {
                Invoke("Launch", i * waveInterval / 60f);
            }
            chargeBackCount = Time.frameCount + (bullentWave - 1) * waveInterval + chargeBack;
        }
    }
